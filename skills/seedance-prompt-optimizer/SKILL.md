...
3. 用户直接发来一段提示词时，先用本技能进行优化：默认返回 FAQ 示例同款紧凑格式：`【全局设定】...`、素材声明行、`镜头1:`、`镜头2:`，再返回简短诊断。
...
生成提示词时使用 FAQ 示例的紧凑格式：

- 第一行：`【全局设定】` 后直接写时长、风格、光影、字幕/音乐/logo/水印等全局约束，不使用 bullet。
- 第二行：如有素材，用 `@图片1是...，@视频1是...，@音频1是...。` 声明职责；无素材则省略。
- 分镜：使用 `镜头1:`、`镜头2:` 连续句式，每个镜头写景别/机位/镜头、主体动作、场景光影和配音/音效。
